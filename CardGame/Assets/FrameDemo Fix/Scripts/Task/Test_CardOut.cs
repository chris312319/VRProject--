...
public class Test_CardOut : MonoBehaviour
{
    ...
    IEnumerator update()
    {
        ...
        if (TestGameFrame.Test_EnemyTask.waitforout == 1)
        {
            ...
            if (now == 0)
            {
                ...
                {
                    count = 0;
                    clip = Resources.Load<AudioClip>("Audios/NPC_小花出牌");
                    Enemy02A.Play("d02");
                    GameAudioController.Instance.PlayOneShot(clip);
                    time = 0;
                    Test_GameData.outcard[TestGameFrame.Test_EnemyTask.j + 3] += 1;
            ...
            else if (now == 1)
            {
                ...
                {
                    count = 0;
                    clip = Resources.Load<AudioClip>("Audios/Host記得出牌");
